...
def get_correlation_coefficent(list_of_dataframe_filepaths):
    ...
    for pairwise_filepaths in list_of_dataframe_filepaths:
        ...
        print(df_ped1.head())
        print(df_ped1["gleitender_MW_15min"].sum(), filename1)
        df_ped1 = cut_nighttime_from_dataframe(df_ped1)
        n1 = df_ped1["count"].sum()
        print(df_ped1.head(), df_ped1.tail())
        ...
        print(df_ped2["gleitender_MW_15min"].sum(), filename2)
        df_ped2 = cut_nighttime_from_dataframe(df_ped2)
        n2 = df_ped2["count"].sum()
        print(df_ped2.head(), df_ped2.tail())
        

        corr_gleitender_MW_15min = df_ped1["gleitender_MW_15min"].corr(df_ped2["gleitender_MW_15min"])
        corr_gleitender_Stundenwert_aus_MW = df_ped1["gleitender_Stundenwert_aus_MW"].corr(df_ped2["gleitender_Stundenwert_aus_MW"])
        ...
        dic = {"zaehlstelle": zaehlstelle , "zaehlstelllen_seite1":zaehlstellen_seite1,  "zaehlstelllen_seite2": zaehlstellen_seite2 
         , "corr_gleitender_MW_15min": corr_gleitender_MW_15min,"corr_gleitender_Stundenwert_aus_MW":corr_gleitender_Stundenwert_aus_MW, "n1 zaehlstelllen_seite1":n1,"n2 zaehlstelllen_seite2": n2, "Summe": n1+n2 }
...
def main():
    ...
    df.to_excel("zaehlstellenseiten_korrelation_unskalierter_werte.xlsx", index=False, engine='openpyxl')
    #print(df)
