...
main_folder_path = r'Z:\_Public\Projekte\IVST\058_FoPS_Fuss\02_Bearbeitung\AP5\09_2Infrastrukturmerkmale\EntfernungKita\Quartile\Stundenmittelwerte'
...
PLOTTITLE = 'Anteil Fußverkehr je Stunde\nunterteilt nach Entfernung Kita in Quantile\ngemittelt'
LEGENDTITLE = 'Legende'
...
for i, subfolder in enumerate(os.listdir(main_folder_path)):
    ...
    if os.path.isdir(subfolder_path):
        ...
        #color = colors[i]
        legend_name = subfolder.split('_')[0]
        ...
        for csv_file in os.listdir(subfolder_path):
            if csv_file.endswith('.csv'):
                ...
                ax.plot(df['start time(ohne Tag)'], df['gleitender_Stundenwert_aus_MW'], color=color_map[subfolder], label=legend_name)
