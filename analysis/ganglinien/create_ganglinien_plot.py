...
main_folder_path = r"Zählstelle_Folders"


PLOTTITLE = 'Anteil Fußverkehr je Stunde\nunterteilt nach Entfernung Bushaltestelle in Quantile\ngemittels'
...
for i, csv_file in enumerate(os.listdir(main_folder_path)):
    if csv_file.endswith('.csv'):
        ...
        # Plot the data
        ax.plot(df['start time(ohne Tag)'], df['gleitender_Stundenwert_aus_MW'], color=color_map[csv_file], label=legend+addon[j]+" n="+str(n))
        j += 1
