...
PLOTTITLE = 'Anteil Fußverkehr je Stunde\nStundenmittelwerte der Zählstellen je RegioStarGem7-Typ'
    ...
    if os.path.isdir(subfolder_path):
        ...
        for csv_file in os.listdir(subfolder_path):
            if csv_file.endswith('.csv'):
                ...
                # Assuming the CSV file has columns 'x' and 'y' // gleitender_MW_15min
                ax.plot(df['start time(ohne Tag)'], df['gleitender_Stundenwert_aus_MW'], color=color_map[subfolder], label=legend_name + " n="+str(n))
